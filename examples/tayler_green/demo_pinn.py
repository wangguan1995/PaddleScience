...
class NSData(ln.Data):
    ...
    def generate(self, num):
        ...
        for key in tlist:
            ...
            p = f[key][:, 4][:, None]
            XYT = np.concatenate(
                (XYT, np.concatenate((XY, np.full((XY.shape[0], 1), time)), axis=1)),
                axis=0,
            )
        ...
        randT = np.random.rand(num["pde"] * n_frame, 1) * (
            float(tlist[-1]) - float(tlist[0])
        ) + float(tlist[0])
...
class PINN(ln.nn.Algorithm):
    ...
    def criterion(self, X, y):
        ...
        vx_xx, vx_yy = (
            grad(vx_x.reshape(-1, 1), z)[:, 0],
            grad(vx_y.reshape(-1, 1), z)[:, 1],
        )
        vy_xx, vy_yy = (
            grad(vy_x.reshape(-1, 1), z)[:, 0],
            grad(vy_y.reshape(-1, 1), z)[:, 1],
        )
        p_x, p_y = p_g[:, 0], p_g[:, 1]
...
def predict(net):
    ...
    for time in t:
        for i in range(0, nn):
            ...
            frame_data[i, :] = np.concatenate(
                (X.reshape(1, -1), Y.reshape(1, -1), predict[None, :]), axis=1
            )
        ...
        scatter = ax.scatter(
            ...
            c=np.sqrt(frame_data[:, 2] ** 2 + frame_data[:, 3] ** 2),
            s=1,
            vmin=0,
            vmax=0.4,
        )
