...
class AE(Map):
    ...
    def __init__(
        self, encoder_size, decoder_size, activation="sigmoid", initializer="default"
    ):
    ...
    def encode(self, x, returnnp=False):
        ...
        return (
            self.ms["encoder"](x).cpu().detach().numpy()
            if returnnp
            else self.ms["encoder"](x)
        )
    ...
    def decode(self, x, returnnp=False):
        if not isinstance(x, torch.Tensor):
            x = torch.tensor(x, dtype=self.dtype, device=self.device)
        return (
            self.ms["decoder"](x).cpu().detach().numpy()
            if returnnp
            else self.ms["decoder"](x)
        )
