...
class LinearModule(Module):
    ...
    def __init_params(self):
        ...
        for i in range(self.layers):
            params["S{}".format(i + 1)] = nn.Parameter(
                (torch.randn([d, d]) * 0.01).requires_grad_(True)
            )
...
class GradientModule(Module):
    ...
    def forward(self, pqh):
        ...
        if self.mode == "up":
            gradH = (
                self.act(q @ self.ps["K"] + self.ps["b"]) * self.ps["a"]
            ) @ self.ps["K"].t()
            return p + gradH * h, q
        elif self.mode == "low":
            gradH = (
                self.act(p @ self.ps["K"] + self.ps["b"]) * self.ps["a"]
            ) @ self.ps["K"].t()
            return p, gradH * h + q
        else:
            raise ValueError
    ...
    def __init_params(self):
        d = self.dim // 2
        params = nn.ParameterDict()
        params["K"] = nn.Parameter(
            (torch.randn([d, self.width]) * 0.01).requires_grad_(True)
        )
        params["a"] = nn.Parameter(
            (torch.randn([self.width]) * 0.01).requires_grad_(True)
        )
...
class ExtendedModule(Module):
    ...
    def forward(self, pqch):
        ...
        if self.mode == "up":
            gradH = (
                self.act(q @ self.ps["K1"] + c @ self.ps["K2"] + self.ps["b"])
                * self.ps["a"]
            ) @ self.ps["K1"].t()
            return p + gradH * h, q, c
        elif self.mode == "low":
            gradH = (
                self.act(p @ self.ps["K1"] + c @ self.ps["K2"] + self.ps["b"])
                * self.ps["a"]
            ) @ self.ps["K1"].t()
            return p, gradH * h + q, c
        else:
    ...
    def __init_params(self):
        ...
        params = nn.ParameterDict()
        params["K1"] = nn.Parameter(
            (torch.randn([d, self.width]) * 0.01).requires_grad_(True)
        )
        params["K2"] = nn.Parameter(
            (torch.randn([dc, self.width]) * 0.01).requires_grad_(True)
        )
        params["a"] = nn.Parameter(
            (torch.randn([self.width]) * 0.01).requires_grad_(True)
        )
...
class LASympNet(SympNet):
    ...
    def __init_modules(self):
        ...
        for i in range(self.layers - 1):
            ...
            modules["ActM{}".format(i + 1)] = ActivationModule(
                self.dim, self.activation, mode
            )
...
class GSympNet(SympNet):
    ...
    def __init_modules(self):
        ...
        for i in range(self.layers):
            mode = "up" if i % 2 == 0 else "low"
            modules["GradM{}".format(i + 1)] = GradientModule(
                self.dim, self.width, self.activation, mode
            )
        return modules
...
class ESympNet(SympNet):
    ...
    def __init_modules(self):
        ...
        for i in range(self.layers):
            mode = "up" if i % 2 == 0 else "low"
            modules["ExtM{}".format(i + 1)] = ExtendedModule(
                self.dim, self.latent_dim, self.width, self.activation, mode
            )
        return modules
