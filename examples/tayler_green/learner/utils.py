    ...
    def wrapper(*args, **kwargs):
        ...
        for arg in args:
            if type(arg) in (list, tuple, dict):
                container, idx = type(arg), arg.keys() if type(arg) == dict else len(
                    arg
                )
                break
        if container is None:
            for value in kwargs.values():
                if type(value) in (list, tuple, dict):
                    container, idx = type(value), value.keys() if type(
                        value
                    ) == dict else len(value)
                    break
        if container is None:
            ...
            get = (
                lambda element, i: element[i] if type(element) is container else element
            )
            return container(
                wrapper(
                    *[get(arg, i) for arg in args],
            ...
            get = (
                lambda element, key: element[key] if type(element) is dict else element
            )
            return {
                key: wrapper(
                    *[get(arg, key) for arg in args],
...
def cross_entropy_loss(y_pred, y_label):
    if y_pred.size() == y_label.size():
        return torch.mean(
            -torch.sum(torch.log_softmax(y_pred, dim=-1) * y_label, dim=-1)
        )
    else:
...
def grad(y, x, create_graph=True, keepdim=False):
    ...
    for i in range(Ny):
        dy.append(
            torch.autograd.grad(
                y[..., i], x, grad_outputs=z, create_graph=create_graph
            )[0]
        )
