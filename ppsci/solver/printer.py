...
def update_train_loss(trainer, loss_dict, batch_size):
    ...
    for key in loss_dict:
        if key not in trainer.train_output_info:
            trainer.train_output_info[key] = AverageMeter(key, "7.5f")
...
def update_eval_loss(trainer, loss_dict, batch_size):
    ...
    for key in loss_dict:
        if key not in trainer.eval_output_info:
            trainer.eval_output_info[key] = AverageMeter(key, "7.5f")
...
def update_eval_metric(trainer, out, batch, batch_size):
    ...
    if trainer.eval_metric_func is not None:
        ...
        for key in metric_dict:
            if key not in trainer.eval_output_info:
                trainer.eval_output_info[key] = AverageMeter(key, "7.5f")
            trainer.eval_output_info[key].update(float(metric_dict[key]), batch_size)
...
def log_train_info(trainer, batch_size, epoch_id, iter_id):
    ...
    time_msg = "s, ".join([
        f"{key}: {trainer.train_time_info[key].avg:.5f}"
        for key in trainer.train_time_info
    ])

    ips_msg = f"ips: {batch_size / trainer.train_time_info['batch_cost'].avg:.5f} samples/s"

    eta_sec = (
        (trainer.cfg["Global"]["epochs"] - epoch_id + 1) *
        trainer.iters_per_epoch - iter_id) * trainer.train_time_info["batch_cost"].avg
...
def log_eval_info(trainer, batch_size, epoch_id, iters_per_epoch, iter_id):
    ...
    time_msg = "s, ".join([
        f"{key}: {trainer.eval_time_info[key].avg:.5f}"
        for key in trainer.eval_time_info
    ])

    ips_msg = f"ips: {batch_size / trainer.eval_time_info['batch_cost'].avg:.5f}" \
              f"samples/s"
