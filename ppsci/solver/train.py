...
from .printer import log_train_info, update_train_loss
...
def train_epoch_func(solver, epoch_id, log_freq):
    ...
    for iter_id in range(1, solver.iters_per_epoch + 1):
        ...
        # backward
        if solver.use_amp:
            total_loss = solver.scaler.scale(total_loss)
            total_loss.backward()
            if iter_id % solver.update_freq == 0:
                solver.scaler.minimize(solver.optimizer, total_loss)
        ...
        update_train_loss(solver, loss_dict, total_batch_size)
        if iter_id == 1 or iter_id % log_freq == 0:
            log_train_info(solver, total_batch_size, epoch_id, iter_id)
