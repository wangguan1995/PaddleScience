...
def save_vtu_from_array(filename, coord, value, value_keys, num_timestamp=1):
    ...
    assert isinstance(coord, np.ndarray), \
        f"type of coord({type(coord)}) should be ndarray."
    assert isinstance(value, np.ndarray), \
        f"type of value({type(value)}) should be ndarray."
    assert len(coord) == len(value), \
        f"coord length({len(coord)}) should be equal to value length({len(value)})"
    assert len(coord) % num_timestamp == 0, \
        f"coord length({len(coord)}) should be an integer multiple of " \
        f"num_timestamp({num_timestamp})"
    assert coord.shape[1] in [2, 3], \
        f"ndim of coord({coord.shape[1]}) should be 2 or 3."
    ...
    for t in range(num_timestamp):
        ...
        if coord_ndim == 2:
            ...
            axis_z = np.zeros([npoint], dtype="float32")
        elif coord_ndim == 3:
            axis_x = np.ascontiguousarray(coord[t*_n:(t+1)*_n, 0])
            axis_y = np.ascontiguousarray(coord[t*_n:(t+1)*_n, 1])
        ...
        for j in range(data_ndim):
            data_vtu[value_keys[j]] = np.ascontiguousarray(value[t*_n:(t+1)*_n, j])

        filename_t = f"{filename}_t-{t}" if num_timestamp > 1 else filename
        pointsToVTK(filename_t, axis_x, axis_y, axis_z, data=data_vtu)
...
def save_vtu_from_dict(filename, data_dict, coord_keys, value_keys, num_timestamp=1):
    ...
    """
    assert len(coord_keys) in [2, 3, 4], \
        f"ndim of coord ({len(coord_keys)}) should be 2, 3 or 4"
    ...
    if value is not None and isinstance(value[0], paddle.Tensor):
