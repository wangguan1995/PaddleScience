...
def parse_args():
    parser = argparse.ArgumentParser("paddlescience train script")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
...
def _is_num_seq(seq):
    ...
    return isinstance(seq, (list, tuple)) and \
        all([isinstance(x, int) for x in seq])
    ...
    if isinstance(node, dict):
        for key in node:
            if replace_shape_with_inputspec_(node[key]):
                node[key] = InputSpec(node[key])
    elif isinstance(node, list):
        for i in range(len(node)):
            if replace_shape_with_inputspec_(node[i]):
                node[i] = InputSpec(node[i])
