...
def EqLoss(eq, netout=None):
    ...
    Example
        ...
        >>> out = net(input)
        >>> loss = ppsci.loss.EqLoss(pde.equations[0], out) # loss is equation loss of equation 0 in pde
    """
...
def BcLoss(name, netout=None):
    ...
    Example:
        ...
        >>> net = ppsci.network.FCNet(...)
        >>> out = net(input)
        >>> loss = ppsci.loss.BcLoss("top", out) # loss is boundary loss on "top" boundary
    """
...
def IcLoss(netout=None):
    ...
    Example:
        ...
        >>> net = ppsci.network.FCNet(...)
        >>> out = net(input)
        >>> loss = ppsci.loss.IcLoss("top") # loss is initial loss
    """
...
def DataLoss(netout=None, ref=None):
    ...
    Example:
        ...
        >>> net = ppsci.network.FCNet(...)
        >>> out = net(input)
        >>> loss = ppsci.loss.DataLoss(out, ref=np.ones(3)) # loss is initial loss
    """
    floss = FormulaLoss()
    floss._suplist = [True]
    floss._supwgt = [1.0]
